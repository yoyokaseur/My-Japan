<template>
  <div class="container_nav">
    <i
      class="fas fa-bars border_line pointer settings_button"
      @click="settings_display"
    ></i>
    <router-link title="Acceuil" class="mark_site border_line pointer" to="/">{{
      mark
    }}</router-link>
    ...
    <button class="change_style" @click="change_style_card">
      <div class="change_style_rond"></div>
    </button>
    <router-link class="About_settings pointer border_line" to="/About"
      >About</router-link
    >
    <router-link class="panier_settings pointer border_line" to="/"
      >Panier</router-link
    >
  </div>
...
let setting_counter : number = 0
let settings_active : boolean = false
let card_style : boolean = true


function settings_display() : void
{
...
function change_style_card() : void
{
    ...
    if (change_style_color == null || change_style_rond == null )
        return
    store.commit("change_style");
    if (card_style == true) {
      change_style_color.setAttribute(
        "style",
        "background: rgb(133, 0, 122)"
      );
